  ...
  return (
    <SectionCard title="Reward Tersedia" description="Tukarkan poin reward Anda dari bank sampah">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {rewards.map((rwd) => {
          const points = Number(rwd.pointsRequired ?? rwd.pointsCost ?? 0);
          const name = rwd.name || rwd.title || 'Reward SCV';
          const stock = Number(rwd.stock || 0);
          const isAffordable = userPoints >= points;
          return (
            <Card key={rwd.rewardId || rwd.id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs flex flex-col justify-between">
              <CardContent className="p-4 space-y-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900 dark:text-slate-100 line-clamp-1">{name}</h4>
                ...
                >
                  {stock <= 0 ? 'Stok Habis' : isAffordable ? 'Tukarkan Poin' : 'Poin Kurang'}
                </Button>
              </CardContent>
            </Card>
          );
